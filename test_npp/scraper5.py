...
BAD_DOMAINS = [
    ...
    "youtube.com", "yelp.com", "glassdoor.com", "indeed.com",
    "bbb.org", "mapquest.com", "popupportal.com",
    # Medical aggregators
    "healthgrades.com", "doximity.com", "vitals.com", "webmd.com",
    "zocdoc.com", "npino.com", "npienumerator.com",
    # News / legal / archive
    "wikipedia.org", "claimdepot.com", "databreach",
    "classaction.org", "topclassactions.com", "hipaajournal.com",
    "classlawdc.com", "federmanlaw.com", "prnewswire.com",
    "nytimes.com", "tiktok.com", "web.archive.org", "recovered.org",
    "simplyhired.com", "pitchbook.com", "causeiq.com", "podchaser.com",
    "businesslist.co.ke",
    # Government / regulatory (wrong sites for finding provider contacts)
    "hhs.gov", "cms.gov", "va.gov", "dhs.gov", "usdoj.gov",
    "jointcommission.org",
    # Data brokers
    "zoominfo.com", "signalhire.com", "leadiq.com",
    # Vendors / unrelated orgs
    "paubox.com", "bluebeam.com", "pnc.com",
]
...
JUNK_DOMAINS = {
    # Aggregators / data brokers
    "zoominfo.com", "signalhire.com", "leadiq.com", "npino.com",
    "doximity.com", "vitals.com", "healthgrades.com", "zocdoc.com",
    "npienumerator.com", "businesslist.co.ke", "causeiq.com",
    "podchaser.com", "simplyhired.com", "pitchbook.com",
    # Government / regulatory bodies — never the provider's own address
    "va.gov", "hhs.gov", "dhs.gov", "usdoj.gov", "cms.gov",
    "cisa.dhs.gov", "cbp.dhs.gov", "fema.dhs.gov",
    "jointcommission.org",
    # Financial / unrelated corporates
    "pnc.com", "bluebeam.com",
    # Insurance / managed care (not the covered entity itself)
    "molinahealthcare.com",
    # Email security / tech vendors
    "paubox.com",
    # Management / billing companies whose address ≠ the provider
    "mhnchicago.org", "eksm.com", "gialliance.com",
    # News, social, archive, job sites
    "prnewswire.com", "tiktok.com", "nytimes.com", "web.archive.org",
    "mass.gov", "pittmandutton.com",
    # Law firms investigating breaches (not the provider)
    "classlawdc.com", "federmanlaw.com", "caffertyclobes.com",
    "recovered.org",
}

# Generic local-parts that appear regardless of domain and always indicate
# a regulatory/complaint address rather than the provider's own contact.
JUNK_LOCAL_PARTS = {
...
def _bare_domain(url):
    """Strip scheme and www. to get a bare hostname for comparison."""
    d = re.sub(r"https?://", "", url or "").split("/")[0].lower()
    return d.lstrip("www.")

def _is_valid_email(email_str, provider_website="", source_url=""):
    """
    Two-stage validation:
      1. Reject if domain is in JUNK_DOMAINS or local-part is in JUNK_LOCAL_PARTS.
      2. Domain-match: the email domain must overlap with EITHER the provider
         website domain OR the source_url domain where it was found. This catches
         third-party addresses while allowing cases where the scraper correctly
         found the right email on a page whose domain differs from the main site
         (e.g. nebraskamed.com email found on a nebraskamed.com page, even though
         the identified website was unmc.edu).
    """
    if not email_str or not email_str.strip():
        return False
    site_domain   = _bare_domain(provider_website)
    ...
    for addr in re.split(r"[;,\s]+", email_str):
        ...
        if local in JUNK_LOCAL_PARTS:
            continue
        # Stage 2: domain-match against website OR source URL
        # Skip only if BOTH are known and NEITHER overlaps with the email domain.
        known_domains = [d for d in (site_domain, source_domain) if d]
        if known_domains:
            if not any(email_domain in kd or kd in email_domain
...
def _scrape_summary(row):
    ...
    has_email = _is_valid_email(row.get("emails", ""), row.get("website", ""), row.get("source_url", ""))
...
def _row_score(r):
    score = 0
    if _is_valid_email(r.get("emails", ""), r.get("website", ""), r.get("source_url", "")): score += 100
...
def build_readme_sheet(wb, stats):
    ...
    removals = [
        ...
        ("Junk email domain filtered (Sheet 4 only)",
         "Two-stage filter: (1) Hard blocklist — domains that are always third-party noise "
         "(government portals, complaint boards, data brokers, law firms, unrelated corporates) "
         "are rejected unconditionally: va.gov, hhs.gov, dhs.gov and subdomains, usdoj.gov, "
         "jointcommission.org, zoominfo.com, signalhire.com, leadiq.com, npino.com, pnc.com, "
         "paubox.com, molinahealthcare.com, mhnchicago.org, eksm.com, gialliance.com, and others. "
         "(2) Domain-match check — even if an email passes the blocklist, it is rejected when "
         "its domain has no overlap with the provider's own website domain. This catches cases "
         "where the scraper followed a link to a third-party page about the provider and picked "
         "up that site's contact address instead. Generic local-parts are also always rejected: "
         "privacyservice, vhalonpublicaffairs, complaint, ocrmail, accommodationrequest, etc."),
...
def build_excel_report(hhs_csv_path, scraped_csv_path):
    ...
    actionable = [r for r in deduped if _is_valid_email(r.get("emails", ""), r.get("website", ""), r.get("source_url", ""))]
    no_contact = [r for r in deduped if not _is_valid_email(r.get("emails", ""), r.get("website", ""), r.get("source_url", ""))]
    ...
    junk_filtered = sum(
        1 for r in deduped
        if r.get("emails","").strip() and not _is_valid_email(r.get("emails",""), r.get("website",""), r.get("source_url",""))
    )
    ...
    for i, row in enumerate(deduped, 1):
        rn = i + 2
        bg = _GRN_BG if _is_valid_email(row.get("emails", ""), row.get("website", ""), row.get("source_url", "")) else _AMB_BG
